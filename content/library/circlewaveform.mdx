...
This component displays a card that reacts to voice during the call. The ring represents the audio volume levels and dynamically animate using framer-motion. Click the mic in the center to start and stop conversation with Voice assistant.
...
Copy the following code to your component file for example **_radial-card.tsx_**. Ensure the use-webrtc hook is copied over to your project.
...
<CodeBlockWrapper size="wrapper">
```tsx {1,6}
import { RadialCard } from "@/components/openai-blocks/radial-card";
